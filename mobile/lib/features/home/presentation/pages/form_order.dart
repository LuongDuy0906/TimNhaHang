...
import 'package:mobile/features/history/domain/usecase/add_bill.dart';
...
class OrderForm extends StatefulWidget {
  ...
  late final _remote = BillsRemoteDataSourceImpl(apiClient: _apiClient);
  late final _repo = BillRepositoryImpl(_remote);
  late final _addBill = AddBill(_repo);

  OrderForm({super.key, required this.resid, required this.uid});
...
class _OrderFormState extends State<OrderForm> {
  ...
  Future<void> _pickBookingTime() async {
    ...
    setState(() {
      ...
      _bookingTimeController.text = _selectedDateTime!.toIso8601String();
    });
  }
  ...
  void _saveBooking() {
    ...
    if (mounted) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Đã đặt bàn thành công')));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Chọn giờ đặt bàn'), centerTitle: true),
      body: Center(
        child: Form(
          key: _formKey,
          child: ListView(
